...
Recommended next action: review the pushed roulette PoC commit and do a browser UX pass for smaller screens, tooltip placement/clipping, chip-selection ergonomics, and status messaging during slow live API/TN10 waits. The rejected API-demo-like scaffold under `apps/roulette-poc/` has been replaced with the two-section target product UI: top `Roulette Game` using the original table-first chip-placement UX from `/root/kaspa-fair-foundation/examples/roulette-poc/ui/`, and `Proof of Fairness` using `apps/roulette-poc/flowchart-spec.json` as a static design/help spec for both a compact live status strip near the table and a collapsible full env095-style flowchart below. Keep `createToccataApiClient` imported from `kaspa-toccata-api`; do not use static round/proof/result JSON as runtime data.
...
- Do not treat the current roulette PoC as complete; it is a first live-API-backed scaffold that still needs browser UX refinement and live-write-gated manual testing.
