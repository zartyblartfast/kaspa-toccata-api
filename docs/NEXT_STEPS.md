...
Recommended next action: read `docs/ROULETTE_UI_DESIGN_HANDOVER.md` and redesign the roulette PoC UI. The current scaffold under `apps/roulette-poc/` proves the browser/npm API import path, but it is too close to the API demo page and is not the target product UI. Target UI: only two main sections — top `Roulette Game` using the original table-first chip-placement UX from `/root/kaspa-fair-foundation/examples/roulette-poc/ui/`, and lower `Proof of Fairness` using the draft flowchart renderer/data from `/root/kaspa-fair-foundation/examples/roulette-poc/ui-sketches/env095-flowchart-*`. Keep `createToccataApiClient` imported from `kaspa-toccata-api`; do not use static round/proof/result JSON as runtime data.
