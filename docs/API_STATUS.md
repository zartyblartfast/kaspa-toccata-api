...
A reusable CommonJS typed client boundary now exists at `src/client.cjs` with TypeScript declarations at `src/client.d.ts`. The package export points at this client. `scripts/client-package-boundary-smoke.sh` verifies a packed local package, and `scripts/npm-client-readiness-smoke.sh` verifies publish-ready tarball contents. The public npm package `kaspa-toccata-api@0.1.1` is published and registry-install verified; proof output includes `tn10Writes` evidence for commit/close/reveal when present.
